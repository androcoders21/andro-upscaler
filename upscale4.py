...
class FluxUpscaler:
    def __init__(self, 
        ...
        # MODIFIED: Use full precision (float32) instead of float16/bfloat16
    ...
    def setup(self):
        ...
        # CRITICAL FIX: Load ControlNet first on second GPU with full precision
        print(f"Loading ControlNet on {self.controlnet_gpu} in full precision")
        controlnet = FluxControlNetModel.from_pretrained(
            self.controlnet_id,
            torch_dtype=torch.float32,  # MODIFIED: Use full precision
        ).to(self.controlnet_gpu)
        
        # First approach: load the pipeline on the main GPU, then move components
        print(f"Loading pipeline on {self.main_gpu} in full precision...")
        
        # Force specified GPU for initial loading
        torch.cuda.set_device(int(self.main_gpu.split(':')[1]))
        
        # MODIFIED: Load the pipeline with full precision
        self.pipe = FluxControlNetPipeline.from_pretrained(
            model_path,
            controlnet=controlnet,  # Already loaded on controlnet_gpu
            torch_dtype=torch.float32,  # MODIFIED: Use full precision
            # MODIFIED: Don't specify variant since we're using full precision
        ...
        if self.offload_vae:
            ...
            # MODIFIED: Just move VAE to CPU, don't change precision
    ...
    def upscale(
        ...
        """Run a single upscaling process with full precision"""
        if seed is None:
        ...
        try:
            ...
            # MODIFIED: Move VAE to main GPU for processing
            if hasattr(self.pipe, 'vae'):
                print("Moving VAE to main GPU for encoding")
                self.pipe.vae = self.pipe.vae.to(self.main_gpu)
            ...
            # MODIFIED: Use full precision inference (remove autocast)
            with torch.inference_mode():
                output = self.pipe(**inference_kwargs)
            
            # Move VAE back to CPU if configured
...
if __name__ == "__main__":
    ...
    parser.add_argument('--token', default="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", help='HuggingFace token')  # MODIFIED: Removed hardcoded token
    parser.add_argument('--offload_vae', action='store_true', help='Offload VAE to CPU when not in use')
    # MODIFIED: Removed float16 option and added full_precision option
    ...
    upscaler = FluxUpscaler(
        ...
        use_float16=False,  #  Use full precision
